...
<div>
  ...
  <template v-if="errorTableData.length">
    ...
    <a-table :dataSource="errorTableData" bordered class="import-error-table" style="width: 100%" rowKey="row">
      <a-table-column title="错误行号" key="row" dataIndex="row" width="180"></a-table-column>
      <a-table-column title="错误原因" key="reason" dataIndex="reason"></a-table-column>
    </a-table>
  </template>
  <!-- 数据列表 -->
  <h1 style="margin-top: 20px;">数据列表</h1>
  <a-table :cell-class-name="checkCell" :dataSource="tableData" :pagination="dataPagination" bordered style="width: 100%" :loading="isLoading" rowKey="key">
    <a-table-column align="center" title="行号" key="key" dataIndex="key" width="50" />

    <a-table-column :key="field" :title="label" :dataIndex="field" align="left" headerAlign="center" v-for="(label, field) of fields" :customRender="(text,record,index) => text" :customCell="checkCells">
      <!-- 自定义错误显示 -->
      <template slot-scope="scope">
        <a-tooltip :content="errorData[scope.$index][field]" class="item" effect="dark" placement="top" v-if="errorData[scope.$index] && errorData[scope.$index][field]">
  ...
  <div class="ant-import-action">
    <a-space>
      ...
      <a-button type="primary" size="large" :loading="isLoading" @click="handleRequest">下一步</a-button>
    </a-space>
  </div>
</div>
</template>
  ...
  methods: {
    ...
    async handleRequest () {
      ...
      try {
        ...
        this.$message.success('导入完成，导入明细请查看详情！')
        this.goNext()
      } catch (error) {
