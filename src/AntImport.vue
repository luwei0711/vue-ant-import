<template>
<div>
  <a-modal :title="title" :visible.sync="visible" :width="dialogWidth" @cancel="handlClose" v-if="visible" :footer="null" :maskClosable="false">
    ...
    <!-- 数据展示 -->
    <ant-import-data :append="append" :fields="fields" :formatter="formatter" :request-fn="requestFn" :rules="rules" :table-data="tableData" @pre="handleStep3Pre" v-if="currentStep === 2" />
      ...
      validator (formatter) {
        for (const key in formatter) {
          if (!(formatter[key] instanceof Object)) {
            ...
            window.console.error(`${key}的值必须为 对象 或 函数`)
            return false
          }
        }
        return true
